...
export function splitGraphemes(text: string): GraphemeUnit[] {
  ...
  // A fresh clone per call: `exec` in a loop mutates `lastIndex`, so the
  // shared exported regex must not be used directly.
  const re = new RegExp(GRAPHEME_SPLIT_REGEX.source, "gu");
...
export function expandColumns(line: Line, tabSize: number): Column[] {
  ...
  for (const token of line.tokens) {
    ...
    for (const [gi, g] of graphemes.entries()) {
