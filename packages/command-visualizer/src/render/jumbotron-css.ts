...
function carouselTrack(state: CascadeState, n: number): string {
  ...
  for (const [j, k] of cmdFrames.slice(1).entries()) {
  ...
  return `@keyframes cmdtrack {${NL}${kf.join(NL)}${NL}}${
    NL
  }.cl-cmd-track { animation: cmdtrack var(--dur, ${String(
    n * MS_PER_STATE,
  )}ms) cubic-bezier(0.4, 0, 0.2, 1) 1 forwards paused; }${NL}`;
}
