...
export function serializeJumbotron(
  ...
  return `<div class="visualizer-wrapper visualizer-jumbotron" data-theme="${state.theme}">${
    NL
  }<div class="jumbotron-container">${NL}${inner}${NL}</div>${NL}${commandBar(
    state,
    opts.barWidthPx,
  )}${NL}${metadataBlock(state)}${NL}${dots(state)}${NL}</div>`;
}
