 ...
 */
function groupFixturesByLanguage(processedFixtures: Array): Record {
 ...
 */
function writeLanguageJsonFiles(
  languageMap: Record,
  outputDir: string,
  debug: boolean,
): Record {
  ...
  Object.entries(languageMap).forEach(([language, fixtures]) => {
    ...
    if (debug) {
      console.log(
        `[yml-to-json] Writing ${fixtures.length} fixtures to ${outputPath}`,
      );
    }
    fs.writeFileSync(
      outputPath,
...
async function main() {
  ...
  if (debug) {
    console.log("🦋 processedFixtures:", JSON.stringify(fixtures, null, 2));
  }
