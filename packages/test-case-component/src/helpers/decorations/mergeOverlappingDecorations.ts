 ...
 */
export function mergeOverlappingDecorations(
  ...
  console.debug(
    "[mergeOverlappingDecorations] decorations:",
  ...
  const zeroWidth = decorations.filter(
    (d) =>
      isPosition(d.start) &&
      isPosition(d.end) &&
      d.start.line === d.end.line &&
      d.start.character === d.end.character,
  );
  ...
  console.debug(
    "[mergeOverlappingDecorations] zeroWidth:",
    JSON.stringify(zeroWidth, null, 2),
  );
  console.debug(
    "[mergeOverlappingDecorations] nonZeroWidth:",
    JSON.stringify(nonZeroWidth, null, 2),
  );
  ...
  for (const p of points) {
    if (
      !uniquePoints.length ||
      uniquePoints[uniquePoints.length - 1].line !== p.line ||
      uniquePoints[uniquePoints.length - 1].character !== p.character
    ) {
      uniquePoints.push(p);
    }
  }
  ...
  for (let i = 0; i < uniquePoints.length - 1; ++i) {
    ...
    const covering = nonZeroWidth.filter(
      (d) =>
        isPosition(d.start) &&
        isPosition(d.end) &&
        (d.start.line < segEnd.line ||
          (d.start.line === segEnd.line &&
            d.start.character < segEnd.character)) &&
        (d.end.line > segStart.line ||
          (d.end.line === segStart.line &&
            d.end.character > segStart.character)),
    );
  ...
  function handleZeroWidthDecoration(
    ...
    console.debug(
      "[handleZeroWidthDecoration] className:",
      className,
      "decoration:",
    ...
    if (
      ...
      if (nextIdx !== undefined) {
        ...
        const newClass =
          typeof nextClass === "string" &&
          nextClass.split(" ").includes("selectionLeft")
            ? nextClass
            : typeof nextClass === "string"
              ? (nextClass + " selectionLeft").trim()
              : "selectionLeft";
        result[nextIdx] = {
          ...next,
          properties: { ...next.properties, class: newClass },
        };
        return true; // handled
        ...
        const newClass =
          ...
          prevClass.split(" ").includes("selectionRight")
            ? prevClass
            : typeof prevClass === "string"
              ? (prevClass + " selectionRight").trim()
              : "selectionRight";
        result[prevIdx] = {
          ...prev,
          properties: { ...prev.properties, class: newClass },
        };
        return true; // handled
      }
      ...
      console.error(
        "[handleZeroWidthDecoration] Unhandled zero-width decoration class:",
        className,
        d,
      );
